...
(load "solutions/Ch.2.2/2.18.scm")


(define (filter include? elems)
  (define (iter rest filtered)
    (if (null? rest)
      filtered
      (let ((elem (car rest)))
        (if (include? elem)
          (iter (cdr rest) (cons elem filtered))
          (iter (cdr rest) filtered)))))
  (reverse (iter elems `())))

(define (same-parity a . rest)
  (let ((parity? (if (odd? a) odd? even?)))
    (cons a (filter parity? rest))))
